...
<h1 class="h3 mb-2 text-gray-800 text-center">Donations </h1>
<div>
    <?php  
...
//     echo $sql1 = "SELECT o.oname, SUM(p.pamt) as pamt 
// FROM tbl_payment p 
// INNER JOIN tbl_event e ON p.eid = e.eid 
// INNER JOIN tbl_organization o ON e.oid = o.oid 
// WHERE p.pstatus = 'donation' 
// AND STR_TO_DATE(p.pdate, '%Y-%m-%d') BETWEEN '$prev_date' AND '$next_date' 
// GROUP BY o.oname";
$sql1 = "SELECT o.oname, SUM(p.pamt) AS pamt
FROM tbl_payment p
INNER JOIN tbl_event e ON p.eid = e.eid
INNER JOIN tbl_organization o ON e.oid = o.oid
WHERE p.pstatus = 'donation' 
  AND (DATE_FORMAT(p.pdate, '%Y-%m-%d') BETWEEN '$prev_date' AND '$next_date')
GROUP BY o.oname";
...
$serial=1;
                        ...
                        <td style="justify-content:center; text-align:center" width="70%"> <?php echo $row["oname"]; echo "Rs";?></td>
                    
                        <td style="justify-content:center; text-align:center" width="70%"> <?php echo $row["pamt"]; echo "Rs";$serial=$serial+1;?></td>
