<?php 
    ...
    $vid=$_SESSION['vid']; 
    $sql_interests="Select * from tbl_volunteerevents where vid=$vid";
        $interests = $db->query($sql_interests);
        // $total=mysqli_num_rows($mentors);
        if($interests->num_rows>0)
        {
            $interest = array();
            while($interest_row=$interests->fetch_assoc())
            {
                $etypeid=$interest_row['etypeid'];
                $interest[] = $etypeid;  
            }
        }
        
if (isset($_POST["submit"])) {
    $selectedEtypeids = $_POST['interests'];
    $deleteQuery = "DELETE FROM tbl_volunteerevents WHERE vid = $vid";
    $db->query($deleteQuery);
    
    foreach ($selectedEtypeids as $etypename) {
        $etypeidQuery = "SELECT etypeid FROM tbl_eventtype WHERE etypename = '$etypename'";
        $result = $db->query($etypeidQuery);
        $row = $result->fetch_assoc();
        $etypeid = $row['etypeid'];
        $insertQuery = "INSERT INTO tbl_volunteerevents (vid, etypeid) VALUES ($vid, $etypeid)";
								...
								<div class="card mt-4  bg-white	">
									...
									<div class="card-body">
										...
										<form action="" method="POST">
											...
											<!-- <label for="exampleInputEmail1">Edit your interests</label> -->
                                            <!-- <select name = 'etypename[]' multiple size = 6>    
													<option value=""></option> -->
                                                    <?php
                                                 $sql_etypename = "SELECT 
                                                 et.etypename,
                                                 CASE WHEN ve.etypeid IS NOT NULL THEN 'selected' ELSE '' END AS selection_status
                                               FROM
                                                 tbl_eventtype et
                                               LEFT JOIN
                                                 tbl_volunteerevents ve ON et.etypeid = ve.etypeid AND ve.vid = $vid
                                               ORDER BY
                                                 et.etypename;
                                               ";
                                                //    echo $sql_etypename;
                                                $result = $db->query($sql_etypename);
                                                while ($row = $result->fetch_assoc()): ?>
                                                    <li>
                                                        <label>
                                                            <input type="checkbox" name="interests[]" value="<?php echo $row['etypename']; ?>" <?php echo ($row['selection_status'] === 'selected') ? 'checked' : ''; ?>>
                                                            <?php echo $row['etypename']; ?>
                                                        </label>
                                                    </li>
                                                <?php endwhile; ?>
                                            </ul>
                                            ...
                                            <button name="submit" type="submit" class="btn btn-primary ml-3 float-right">Update</button>
