...
<h1 class="h3 mb-2 text-gray-800 text-center">Activities</h1>
<div>
    <?php  
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <?php
                        ...
                        // $sql1 = "SELECT tbl_activity.ayid, tbl_activity.eid, tbl_activity.aystatus, tbl_event.oid, tbl_event.eid, tbl_event.edescription, tbl_event.estart, tbl_event.elocation, tbl_event.ewage
                        // FROM tbl_activity
                        // JOIN tbl_event ON tbl_activity.eid = tbl_event.eid
                        // WHERE tbl_activity.vid = $vid";
               
                        $sql1 = "SELECT tbl_activity.ayid, tbl_activity.eid, tbl_activity.aystatus, tbl_event.oid, tbl_event.eid, tbl_event.edescription, tbl_event.estart, tbl_event.elocation, tbl_event.ewage, tbl_payment.pstatus
                        FROM tbl_activity
                        JOIN tbl_event ON tbl_activity.eid = tbl_event.eid
                        JOIN tbl_payment ON tbl_activity.eid = tbl_payment.eid AND tbl_activity.vid = tbl_payment.vid
                        WHERE tbl_activity.vid = $vid";
                 ...
                 if ($result1->num_rows > 0) {
                        ...
                        <tr>
                            ...
                            <th style="text-align:center;"><b>Amount Received</b></th>
                            <th style="text-align:center;"><b>Certificate</b></th>
                        ...
                        <td style="justify-content:center; text-align:center" width="70%"> <?php echo $row["aystatus"]; ?> </td>
                        <!-- <td style="justify-content:center; text-align:center" width="70%"> <?php echo $row["ewage"]; echo "Rs"; ?> </td> -->
                        <td style="justify-content:center; text-align:center" width="10%"> 
    <?php 
        if ($row["aystatus"] == 'complete' && $row["pstatus"] == 'paid') {
            echo $row["ewage"] . "Rs";
        }
        else if ($row["aystatus"] == 'complete' && $row["pstatus"] == 'donation') {
            echo "Donation";
            // if status = finished, display download, else suggest something to display as there is no certificate to download
            // echo '<span style="color:maroon" class="glyphicon glyphicon-edit fas fa-lg mt-1 py-2 fa-times" aria-hidden="true"></span>';
        }
...
<a href="certificate.php?eid=<?php echo $row["eid"] ?>"><input type="button" value="download" class="btn btn-success float-right"></a>

                            <?php
                                }
                             
                            
                            else if($row["aystatus"] =='incomplete') { echo "";?> 
                            <!--if status = finished, display download, else ? suggest something to display as there is no certificate to download
